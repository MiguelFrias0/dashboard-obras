...
# ==========================================
# A MÁGICA DA SEGURANÇA: Chave puxada do cofre
# ==========================================
...
try:
    ...
    # 2. Zona Autônoma (Trimestre)
    inicio_tri = hoje - timedelta(days=90)
    semanas_tri = 90 / 7 
    ...
    if isinstance(periodo, tuple) and len(periodo) == 2:
        ...
        # --- CÁLCULOS DO TRIMESTRE (MÉDIAS SEMANAIS) ---
        mask_desp_tri = (df_raw['DESPACHADO'] >= inicio_tri) & (df_raw['DESPACHADO'] <= hoje)
        total_desp_tri = df_raw[mask_desp_tri].shape[0]

        mask_env_tri = (df_raw['ENVIADO'] >= inicio_tri) & (df_raw['ENVIADO'] <= hoje) & (status_f.str.contains('ok', na=False))
        ...
        # --- METAS SEMANAIS (Baseado na média do trimestre) ---
        ...
        st.subheader("🎯 Desempenho e Metas (Base: Últimos 90 dias)")
